...
const FEED_PATH = path.resolve(
  process.env.FEED_PATH ?? `${process.cwd()}/psf-feed.atom`
);
...
interface BloggerEntry {
  id: string;
  // Takeout export fields (may be absent in API feed)
  "blogger:type"?: string;
...
async function migrate() {
  console.log("🐍 PSF Blog Migration");
  ...
  // Takeout exports have blogger:type/blogger:status; API feeds don't (all entries are live posts)
  const isTakeout = entries.length > 0 && entries[0]["blogger:type"] != null;
  const livePosts = isTakeout
    ? entries.filter((e) => e["blogger:type"] === "POST" && e["blogger:status"] === "LIVE")
    : entries;

  console.log(`Live posts to migrate: ${livePosts.length} (${isTakeout ? "Takeout" : "API"} format)`);
  ...
  for (const entry of livePosts) {
    try {
      ...
      // Legacy URL: from blogger:filename (Takeout) or <link rel="alternate"> (API)
      let legacyFilename = entry["blogger:filename"] || "";
      if (!legacyFilename) {
        const links = Array.isArray(entry.link) ? entry.link : entry.link ? [entry.link] : [];
        const alt = links.find((l) => l["@_rel"] === "alternate" && l["@_type"] === "text/html");
        if (alt?.["@_href"]) {
          try {
            legacyFilename = new URL(alt["@_href"]).pathname;
          } catch { /* ignore */ }
        }
      }
